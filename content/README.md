...
5분판이라 관문은 3문제 중 **2문제**, 조립 중 확인은 2문제 중 **1문제**만 출제한다
(보스 4문제는 실물 해체 4단계라 전부 낸다). **안 나오는 문제도 지우지 말 것** —
회차마다 다른 문제가 뽑히고, 10분판으로 돌아갈 때 그대로 쓴다.

출제 수는 텍스트가 아니라 진행 설정이라 JSON 이 아닌 `src/data/quiz.js` 의 `S.ASK` 에 있다.
