...
게임에 나오는 **모든 글자**는 이 폴더의 JSON에서 온다. 코드를 건드릴 필요가 없다.
파일 이름은 `<종류>.<언어>.json`.

| 파일 | 담당 | 예시 |
|---|---|---|
| `ui.*.json` | **화면 텍스트** — 버튼·라벨·가이드 문구·엔딩·폰트 설정 | `다음 →`, `부품 찾기` |
| `dialogue.*.json` | **대사** — NPC가 말하는 것 + 오프닝 내레이션 | `잘 왔구나, 견습 메카닉.` |
| `parts.*.json` | **부품 도감** 15종 | `허브 왕`, `로봇의 두뇌이자 심장.` |
| `quiz.*.json` | **퀴즈** 9문제 + 해설 | `허브의 포트는 몇 개?` |

> 나누는 기준: 사람이 입으로 말하면 **dialogue**, 화면에 붙어 있으면 **ui**,
> 도감 항목이면 **parts**, 문제면 **quiz**.

현재 언어는 `ko`(한국어)와 `en`(English) 둘이다.

## 언어 전환

타이틀 화면 우측 상단의 **언어 버튼**을 누르거나 **`L`** 키.
강사가 한 번 정해두면 학생이 바뀌어도(Ctrl+R) 유지된다.

새 언어를 추가하려면 4개 파일을 복사해 번역하고
(`ui.ja.json` 처럼) `index.dev.html` 에 태그 4줄을 추가한 뒤 `npm run build`.

## 퀴즈 정답 표기

`choices` 의 **첫 번째가 정답**이다. 게임이 출제할 때 보기 순서를 섞으므로
학생에게는 티나지 않는다. 번역할 때 정답 번호를 따로 맞출 필요가 없다.
...
「넘침 없음」이 나오면 안전합니다. UI·대사 JSON 뿐 아니라
`src/data/parts.js`(도감)와 `src/data/quiz.js`(퀴즈)까지 함께 봅니다.
